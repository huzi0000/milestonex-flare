...
function ProductFlow() {
  const steps = [
    { icon: ShieldCheck, number: "01", title: "Protect", copy: "Client locks the live FTSO-priced FXRP amount in escrow." },
    { icon: FileKey2, number: "02", title: "Prove", copy: "Contractor commits a tamper-evident delivery hash on Coston2." },
    { icon: HandCoins, number: "03", title: "Release", copy: "Approved value moves directly to the contractor with a public receipt." },
  ];
  return (
    <div className="product-flow">
      <div className="product-flow-head"><span>HOW MILESTONEX WORKS</span><strong>Agreement to payment in three verifiable moves.</strong></div>
      <div className="product-flow-grid">
        {steps.map(({ icon: Icon, number, title, copy }) => (
          <div key={number}><span className="flow-icon"><Icon size={18} /></span><p><small>{number}</small><strong>{title}</strong><span>{copy}</span></p></div>
        ))}
      </div>
    </div>
  );
  ...
  return (
    <>
      <section className="page-heading dashboard-heading">
        ...
        <div className="hero-actions"><a className="primary-button" href="/lifecycle.html"><Zap size={17} /> Open live workflow</a><button className="secondary-button" onClick={onCreate}><Plus size={17} /> Preview project builder</button></div>
      </section>
  ...
  return (
    <div className="app-shell">
      ...
      <div className="main-column">
        ...
        <footer><BrandLogo /><p>Experimental Coston2 prototype. Never use real funds.</p><div><a href="https://github.com/huzi0000/milestonex-flare" target="_blank" rel="noreferrer">GitHub <ExternalLink size={12} /></a><a href={COSTON2_EXPLORER} target="_blank" rel="noreferrer">Explorer <ExternalLink size={12} /></a></div></footer>
      </div>
    </div>
  );
}
